...
final class PngquantPostProcessorTest extends TestCase
{
    private PngquantPostProcessor $processor;

    protected function setUp(): void
    {
        $bin = (new ExecutableFinder())->find('pngquant');
        if (null === $bin) {
            self::markTestSkipped('pngquant binary is not installed.');
        }

        $this->processor = new PngquantPostProcessor($bin);
    }

    public function testSupportsOnlyPng(): void
    {
        self::assertTrue($this->processor->supports(OutputFormat::Png));
    ...
    public function testProcessesAValidPngImage(): void
    {
        $result = $this->processor->process(new GeneratedImage($this->samplePngBytes(), OutputFormat::Png));

        self::assertSame(OutputFormat::Png, $result->format);
    ...
    public function testThrowsOnInvalidInput(): void
    {
        ...
        $this->processor->process(new GeneratedImage('not a real image', OutputFormat::Png));
    }
